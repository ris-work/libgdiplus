...
double GDIPLUS_MEASURETEXT_PIXELS_FIX = 1.0;
...
static inline const char *gdiplus_get_font_path(void) {
    ...
    return (font_env && font_env[0] != '\0') ? font_env
...
static void init_gdiplus_measuretext_pixels_fix(void) {
    const char *env = getenv("GDIPLUS_MEASURETEXT_PIXELS_FIX");
    if (env) {
        char *endptr;
        double val = strtod(env, &endptr);

        /* strtod sets endptr==env if no conversion happened,
           and *endptr=='\0' if the entire string was valid. */
        if (endptr != env && *endptr == '\0') {
            GDIPLUS_MEASURETEXT_PIXELS_FIX = val;
            fprintf(stderr, "GDIPLUS_MEASURETEXT_PIXELS_FIX: %lf",
                    GDIPLUS_MEASURETEXT_PIXELS_FIX);
        } else {
            fprintf(stderr,
                    "Warning: invalid GDIPLUS_MEASURETEXT_PIXELS_FIX=\"%s\"\n",
                    env);
            /* retain the default */
        }
    }
 ...
 * init_text_shaping
 *
 * Initializes FreeType and HarfBuzz using the font path from
 * gdiplus_get_font_path() (or default "NotoSans-Regular.ttf"). It also sets the
 * default pixel size and retrieves the extra character spacing factor from the
 * environment. This function is guarded so that initialization occurs only
 * once.
 *
...
static inline void init_text_shaping(void) {
    if (g_text_shaping_initialized)
        return;
    init_gdiplus_measuretext_pixels_fix();
    ...
    cairo_font_face_t *cairo_face =
        cairo_ft_font_face_create_for_ft_face(g_ft_face, 0);
    ...
    // Optionally override the default pixel size using the "GDIPLUS_FONT_SIZE"
...
/*
 * If your version of Cairo doesn't have cairo_get_font_size(), supply a
 * fallback. This fallback assumes the font matrix scaling is uniform.
 */
 ...
 * RenderShapedText
 *
 * Shapes the given UTF-8 text with HarfBuzz and renders the resulting glyphs
 * with Cairo, applying an extra character spacing.
 *
 ...
 */
static inline void RenderShapedText(cairo_t *ct, const char *text,
                                    hb_font_t *hb_font,
                                    hb_direction_t direction, double startX,
                                    double startY, double FontSize) {
    ...
    float desiredSizeF = l_pixel_size * (3.0 / 4.0) * FontSize / 12.0;
